...
const StudentProfilePage = () => {
  ...
  const isOwnProfile = !routeId; // if routeId present, viewing someone else
  const type = 'student';
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const normalized = {
        ...
        cgpa: typeof p.cgpa === 'number' ? p.cgpa : p.cgpa ? Number(p.cgpa) : null,
        category: p.category ?? '',
      };
  ...
  const validate = () => {
    ...
    if (merged.cgpa !== null && merged.cgpa !== undefined && (Number.isNaN(merged.cgpa) || merged.cgpa < 0 || merged.cgpa > 10)) return 'CGPA must be between 0 and 10';
    return null;
  };

  const onChange = (changes) => {
    if (!isOwnProfile) return; // prevent editing when viewing others
  ...
  const onSave = async () => {
    ...
    try {
      ...
      const normalized = {
        ...
        cgpa: typeof saved.cgpa === 'number' ? saved.cgpa : saved.cgpa ? Number(saved.cgpa) : merged.cgpa ?? null,
